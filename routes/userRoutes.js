...
router.put('/profile', ensureAuthenticated, attachUserData, async (req, res) => {
    try {
        const userId = req.user.id;
        const { fullName, profileImage, bio } = req.body;
        const updatedUser = await User.findByIdAndUpdate(userId, { fullName, profileImage, bio }, { new: true });
